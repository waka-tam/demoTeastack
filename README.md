...
## 利用方法
以下「開発環境の設定」に従ってコマンドを実行すると静的Webサイト構築で必要な環境が整います。
...
### 開発環境の設定
1. パッケージのインストール
パッケージマネジャーにnpmもしくはYarnを使用します。特別な理由が無い場合はYarnの利用を推奨します。
...
ex) mediaQuery.ts メディアクエリの判定を行います。
詳しくは以下ディレクトリのファイル内コメントを参照してください。
...
### 画像命名規則
画像ファイル、フォルダ名で使用する文字記号は「半角英数字・ハイフン・アンダースコア」のみとしてください。
全角やその他記号を利用した場合、文字化けなどのバグが発生する恐れがあります。

#### 共通画像の命名
サイト全体で使用するロゴ画像やアイコンなどはshareフォルダに格納します。
以下ディレクトリを参照してください。

**src/assets/img/share/**

共通画像の命名は以下フォーマットに従ってください。

**/share/share + "_" +（内容が推測可能な任意の文字列）+ "(複数種類あれば連番 or 種類を表す任意の文字列)" + ".（拡張子）"**

サンプルで share_bg_01.jpg が格納されていますので開発時には削除してください。

#### 画面固有画像の命名
ページ単位で使用するロゴ画像やアイコンなどはpageフォルダ以下にページ名のフォルダを作成して画像を格納します。
sampleというページで利用する画像であればsampleフォルダを作成します。
以下ディレクトリを参照してください。

**src/assets/img/page/**

ページ単位で使用する画像の命名は以下フォーマットに従ってください。

**/page/ページ名フォルダ/（ページ名）+ "_" +（内容が推測可能な任意の文字列）+ "_" + （複数種類あれば連番 or 種類を表す任意の文字列）+ ".（拡張子）"**
